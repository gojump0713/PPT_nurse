...
## 0. 개인정보 처리 — S14 제주대 운영 사진

발주 측이 제공한 원본(`reference/제주 약학대.png`)에는 **응시생 34명의 실명과 학번**이
모니터링 대시보드에 그대로 노출되어 있었습니다. 다른 대학 학생의 식별정보를
영남이공대·영남대병원 청중에게 보여주게 되므로 해당 영역만 모자이크했습니다.

| 대상 | 처리 |
|---|---|
| 좌상단 대시보드 — 이름 + 학번 그리드 | 모자이크 (34대 감시 구조는 그대로 보임) |
| 좌하단 강의실 화면 속 대시보드 | 모자이크 |
| Notepad.exe 경고 카드의 학번 | 검은 바 (이름은 원본에서 이미 가려져 있었음) |

배포본 파일은 `assets/images/ref/jeju-pharm-cbt.webp` 이고, 화면에도
「응시생 식별정보는 마스킹 처리」라고 표기됩니다.

제주대학교로부터 원문 사용 동의를 받았다면, 같은 경로에 원본을 넣고 캡션의
마스킹 표기를 지우면 됩니다. 동의 없이 되돌리지 마십시오.
