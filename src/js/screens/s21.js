...
export function create() {
  ...
  const cards = AREAS.map((a) => {
    const itemEls = a.items.map((t) =>
      h('li.s21__item', h('span.s21__check', '✓'), t)
    );
    const card = rv('up', 'div.s21__card',
      h('div.s21__card-head',
        icons[a.icon]({ size: 34, className: 's21__icon' }),
        h('span.s21__card-title', a.key),
        h('span.s21__card-count', `${a.items.length}`)
      ),
      h('ul.s21__items', ...itemEls)
    );
    card.itemEls = itemEls;
    return card;
  });

  const grid = h('div.s21__grid', ...cards);

  const counter = h('div.s21__counter',
    h('span.s21__counter-n', '4'), '개 영역 · 후보 업무 ',
    h('span.s21__counter-n', String(TOTAL)), '개'
  );

  const cta = CTA('우선순위 진단 미팅 — 전산부서와 함께', { solid: false });
  const ctaBar = h('div.s21__cta', cta);

  const el = ScreenRoot(meta, { className: 's21' },
    h('div.s21__top', gov, counter),
    grid,
    ctaBar
  );
