...
export function create() {
  ...
  const mapWrap = rv('scale', 'div.s22__map', map, ...pops);

  const purposeEls = PURPOSE.map((p, i) =>
    h('div.s22__purpose', h('span.s22__purpose-no', String(i + 1).padStart(2, '0')), p)
  );
  const purposes = h('div.s22__purposes',
    h('span.s22__purposes-label', '공통 목적'),
    ...purposeEls
  );
  ...
  const el = ScreenRoot(meta, { className: 's22' },
    gov,
    mapWrap,
    purposes,
    foot
