/**
 * SCREEN 22 (PART 전환 · 브리지 ②) — CBT · VDI 공통 기반 구조
...
export function create() {
  ...
  const header = rv('down', 'header.hdr.s22__hdr',
    h('h1.hdr__title', meta.title),
    h('div.s22__parts',
      h('span.s22__part.is-from', h('b', 'PART 2'), 'VDI'),
      h('span.s22__part-arrow', '→'),
      h('span.s22__part.is-to', h('b', 'PART 3'), 'TILON')
    )
  );

  const cards = USERS.map((u, i) =>
    h('div.s22__card', { dataset: { i: String(i), part: u.part } },
      icons[u.icon]({ size: 36, className: 's22__card-icon' }),
      h('div.s22__card-role', u.role),
      h('div.s22__card-value', u.value)
    )
  );

  const keyEls = BASE.map((b) => h('span.s22__key', b));
  const plate = h('div.s22__plate',
    h('div.s22__plate-inner', ...keyEls.flatMap((k, i) => (i > 0 ? [h('span.s22__plus', '+'), k] : [k]))),
    h('div.s22__plate-label', 'TILON Technology Base')
  );

  const gov = Governing(meta.governing, { size: 'sm', center: true });

  const el = ScreenRoot(meta, { header: false, className: 's22' },
    header,
    h('div.s22__body',
      h('div.s22__cards', ...cards),
      plate,
      gov
    )
  );
