/**
 * SCREEN 34 (클로징) — 학생 · 의료진 디지털 환경 제안
...
export function create() {
  const makeTrack = (spec, side) => {
    const nodeEls = spec.nodes.map((t, i) =>
      h(`div.s34__node${i === spec.nodes.length - 1 ? '.is-last' : ''}`, t)
    );
    const track = h(`div.s34__track.s34__track--${side}`,
      h('div.s34__org', spec.org),
      h('div.s34__nodes',
        ...nodeEls.flatMap((n, i) => (i > 0 ? [h('span.s34__node-arrow', '→'), n] : [n]))
      )
    );
  ...
  const logo = h('div.s34__logo',
    h('img', { src: 'assets/images/brand/tilon-ci.png', alt: 'TILON' })
  );

  const converge = h('svg.s34__converge', { viewBox: '0 0 1680 200', preserveAspectRatio: 'none' },
    h('path.s34__cline', { d: 'M120,20 C 480,20 620,150 840,150', fill: 'none' }),
    h('path.s34__cline', { d: 'M1560,20 C 1200,20 1060,150 840,150', fill: 'none' })
  );

  const msg1 = rv('up', 'p.s34__msg',
    '학생에게는, 국가시험에서 처음 만나는 CBT가 아니라 ',
    h('em', '이미 익숙한 시험환경'), '을.'
  );
  const msg2 = rv('up', 'p.s34__msg',
    '의료진에게는, 보안 때문에 불편한 환경이 아니라 ',
    h('em', '보안되어 있기에 자유로운 업무환경'), '을.'
  );

  const ending = rv('up', 'div.s34__ending',
    h('span.s34__ending-lead', 'Digital & AI Transformation'),
    h('span.s34__ending-main', '일하고 배우는 방식을 바꿉니다.'),
